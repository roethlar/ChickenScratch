...
- **Branch**: `fix/m-2-corrupt-sidecars` (REOPENED — see `.review/results/M-2.reopened.md`)
...
- **Reviewer comments (REOPEN)**: `.meta` strict parse + pre-flight validation is correct (writer.rs:288 runs over every doc before `write_project_metadata`), and `record_daily_words` / `parse_writing_history` are tight. **But `get_session_progress` at `src-tauri/src/commands/io.rs:533-536` still uses `fs::read_to_string().ok().and_then(serde_json::from_str().ok()).unwrap_or_default()`** — the exact silent-default shape M-2 was supposed to retire. This contradicts your writeup ("read and write paths share the same parser") and creates a divergence: `record_daily_words` refuses to write a corrupt history while `get_session_progress` happily reports zero today-words on the same corrupt file. Fix: replace with a call to `parse_writing_history`, add a test for the session-progress corruption path, push a fix-up commit on the same branch, write a fresh sentinel. Also flagged for follow-up (out of scope): `settings.rs:354-357` and `:517-520` still use silent-default parses (app-level, not project-sidecar) — track separately as `M-2-followup-app-settings`.
