...
### R-21: Duplicate document IDs/paths silently alias or overwrite content `[x]`
...
- **Reviewer verdict**: VERIFIED via merge commit `8395df2`. Full identity-collision matrix covered: `test_read_project_rejects_duplicate_hierarchy_document_paths` (project.yaml level), `test_read_project_rejects_duplicate_document_ids` (`.meta` level), `test_read_project_rejects_hierarchy_id_path_mismatch` (cross-check between hierarchy and disk), `test_write_project_rejects_duplicate_document_paths_before_write` (write-time guard before any file is touched). Critical property: writer rejects BEFORE writing so a malformed in-memory state cannot escape to disk — paired with R-15 atomic writes, this means duplicates can't corrupt across save windows either. Hierarchy missing-body case is left as a permitted state so R-19's non-destructive missing-file behavior is preserved. Linux native creation now uses `utils::slug::unique_slug` so duplicate titles produce `chapter-one.md` and `chapter-one-1.md`, matching Tauri behavior. PKGBUILD pre-pinned `f61e941…1647`. 4/4 new tests pass.
...
### R-23: Manual Backup reports success while omitting current work `[x]`
...
- **Reviewer verdict**: VERIFIED via merge commit `a45f2c8`. Full fix chain works end-to-end: Backup button (`Revisions.tsx:161`) goes through `runWithEditorFlush("Backup", ...)` → calls `gitCmd.manualBackup` → Tauri `manual_backup` command runs under the per-project write lock → core `backup_current_work` commits dirty state with "Manual backup" message before pushing. Two tests cover both legs: `manual_backup_commits_dirty_work_before_pushing` (headline scenario) and `manual_backup_pushes_clean_head_without_empty_revision` (no empty-commit spam when nothing's dirty). The write-lock scoping means a user can't race a concurrent autosave with a backup. PKGBUILD pre-pinned `c24fe59…23cc`.

### R-24: Document/flow switches do not await failed flushes before replacing buffers and clearing dirty state `[x]`
...
- **Branch**: `fix/r-24-navigation-flush`
- **Approach**: add guarded editor navigation helpers that await the pending editor flush before document selection, selection clear, flow entry, or flow exit; update all document/flow navigation call sites to use them; and make the editor load effect await its fallback flush before replacing/clearing the buffer or dirty state.
- **Tests**: UI lint/build; `cargo fmt --all -- --check`; clippy; full Rust suite; release metadata gate before review handoff.
- **Files changed**: `ui/src/components/editor/navigationGuards.ts`, `ui/src/components/editor/Editor.tsx`, `ui/src/components/editor/Toolbar.tsx`, `ui/src/components/binder/Binder.tsx`, `ui/src/components/corkboard/Corkboard.tsx`, `ui/src/components/search/ProjectSearch.tsx`, `ui/src/components/command-palette/CommandPalette.tsx`, `ui/src/components/stats/StatsPanel.tsx`, `ui/src/components/timeline/TimelineView.tsx`, `ui/src/components/revisions/Revisions.tsx`, `.review/findings/R-24.md`, `REVIEW.md`, `pkg/arch/PKGBUILD`.
...
### R-25: AI replacement deletes selected prose before the stream succeeds `[x]`
...
- **Reviewer verdict**: VERIFIED via merge commit `f7d7534`. All-or-nothing replacement pattern: stream to in-memory buffer, then commit single `insertContentAt({from,to}, replacement)` transaction only if all five guards hold — not aborted, not nav-cancelled, non-empty replacement, still on origin doc, AND **both** `textBetween(from,to) === selectedText` AND `selectedRangeFingerprint === selectedFingerprint`. The fingerprint check is the careful part: catches concurrent edits that produce identical plain text but different ProseMirror slice structure (e.g., a link annotation added during the stream) — without it, autosave could persist a replacement against drifted marks. On any failure or guard reject, the original selection is preserved untouched. Brainstorm unchanged (it appends, no replacement). PKGBUILD pre-pinned `a426057…3853`.
...
### R-27: Rust release/package builds do not enforce `Cargo.lock` `[x]`
...
- **Branch**: `fix/r-27-cargo-lock-enforcement`
- **Approach**: add `cargo metadata --locked --format-version 1` to the release metadata gate and Tauri bundle CI, run validation clippy/tests with `--locked`, pass `--locked` through Tauri bundle builds as trailing Cargo args, and make the Arch package build use `cargo build --locked`.
- **Tests**: locked metadata preflight; release metadata gate; `cargo fmt --all -- --check`; locked clippy; locked full Rust suite; UI lint/build.
- **Files changed**: `scripts/check-release-metadata.sh`, `.github/workflows/validation.yml`, `.github/workflows/tauri-bundles.yml`, `RELEASE.md`, `pkg/arch/PKGBUILD`, `.review/findings/R-27.md`, `REVIEW.md`.
- **Known gaps**: native Tauri bundle artifacts and Arch `makepkg` still need CI/native hosts; this branch enforces locked Cargo resolution in those commands. The first locked full Rust suite attempt hit the existing flaky `utils::process::tests::process_helper_caps_combined_stdout_and_stderr`; the immediate rerun passed the full locked suite.
- **Reviewer verdict**: VERIFIED via merge commit `4e7a1b6`. `--locked` now wired through every release-path Cargo invocation: `check-release-metadata.sh:56` (`cargo metadata --locked --format-version 1` — fails release gate on stale lock), `validation.yml` (clippy + tests `--locked`), `tauri-bundles.yml` (explicit "Locked Rust dependency preflight" step on both macOS and Linux jobs, plus `cargo tauri build ... -- --locked` with trailing `--` to pass the flag through Tauri CLI to the underlying cargo build), `PKGBUILD` (`cargo build --locked --release`). Gate passes locally — Cargo.lock is in sync with manifests today. Any future manifest edit that doesn't refresh the lockfile will now fail at PR time via the validation workflow. R-30's flaky `process_helper_caps_combined_stdout_and_stderr` is the known process-tree timing issue — orthogonal to R-27. PKGBUILD pre-pinned `370be2a…26a3`.

### R-28: Native Swift and Windows dependency resolution is not locked `[x]`
...
- **Branch**: `fix/r-28-native-lockfiles`
- **Approach**: track `macos/Package.resolved`, stop ignoring it, run the Swift cross-frontend harness with automatic resolution disabled, pin the floating WindowsAppSDK reference, enable and check in NuGet lock files for the app/core/harness projects, restore with NuGet locked mode in CI, and add a wildcard package-reference guard.
- **Tests**: SwiftPM lock resolve + diff check; NuGet locked restore; wildcard scan fallback; cross-frontend harness with Swift and C# writers; Rust fmt/clippy/tests with `--locked`; UI install/lint/build; release metadata; Windows core and harness Release builds.
- **Files changed**: `.github/workflows/validation.yml`, `.github/workflows/windows.yml`, `.gitignore`, `RELEASE.md`, `crates/core/tests/cross_frontend/run.sh`, `macos/Package.resolved`, `pkg/arch/PKGBUILD`, `scripts/check-nuget-package-versions.ps1`, `windows/ChickenScratch.App/ChickenScratch.App.csproj`, `windows/ChickenScratch.App/packages.lock.json`, `windows/ChickenScratch.Core/packages.lock.json`, `windows/ChickenScratch.Core.Tests/CrossFrontendHarness/packages.lock.json`, `windows/ChickenScratch.Core.Tests/GitServiceRestoreHarness/packages.lock.json`, `windows/Directory.Build.props`, `.review/findings/R-28.md`, `REVIEW.md`.
- **Known gaps**: local host lacks `pwsh`, so the new PowerShell guard will execute in CI and was locally checked with an equivalent `rg` wildcard scan. Full WinUI app build still needs Windows; the macOS attempt reaches the Windows-only XAML compiler while core/harness projects build locally. The first locked Rust suite hit the known R-30 process-helper flake; the immediate full rerun passed.
- **Reviewer verdict**: VERIFIED via merge commit `4486c91`. SwiftPM: `Package.resolved` checked in, harness uses `--disable-automatic-resolution`, `swift package resolve` produces no diff against the committed file (lock is in sync). NuGet: `WindowsAppSDK 1.8.*` → pinned `1.8.260508005`; 4 `packages.lock.json` files committed for app + core + 2 harness projects; `Directory.Build.props` enables lockfile mode; `dotnet restore --locked-mode` succeeds locally ("All projects are up-to-date"); both `windows.yml` and `validation.yml` run restore with `--locked-mode`; new `check-nuget-package-versions.ps1` blocks wildcards (locally equivalent `rg` scan confirms zero `Version="*"` patterns in `*.csproj`/`*.props`/`*.targets`). PKGBUILD pre-pinned `2dc959b…6a76`.

### R-29: macOS DMG release path has no signing/notarization gate `[x]`
...
- **Branch**: `fix/r-29-macos-signing-notarization`
- **Approach**: kept the existing macOS bundle workflow as an explicitly unsigned smoke build and added a dedicated `macOS Signed Release` workflow triggered by `v*` tags or `workflow_dispatch` in the protected `release-macos` environment. The release workflow fails before building if Apple signing/notarization secrets are missing, imports the Developer ID Application certificate into a temporary keychain, writes the App Store Connect API key to `APPLE_API_KEY_PATH`, builds app/dmg bundles through locked Tauri, explicitly submits and staples the DMG with `xcrun notarytool`, and uploads artifacts only after `codesign`, `spctl`, and `stapler` verification passes. `tauri.conf.json` now explicitly declares hardened runtime for macOS bundles, and release docs/roadmap distinguish unsigned smoke artifacts from public signed/notarized artifacts.
- **Tests**: workflow YAML parse; `git diff --check`; locked cargo metadata; frontend install; unsigned macOS smoke `CI=true cargo tauri build --bundles app,dmg -- --locked`; app and DMG artifact checks; release metadata/checksum gate.
- **Files changed**: `.github/workflows/macos-release.yml`, `.github/workflows/tauri-bundles.yml`, `RELEASE.md`, `docs/ROADMAP.md`, `src-tauri/tauri.conf.json`, `pkg/arch/PKGBUILD`, `.review/findings/R-29.md`, `REVIEW.md`.
- **Known gaps**: the full signed/notarized path requires Apple Developer credentials and therefore is enforced in the protected GitHub Actions release job rather than locally. No app-sandbox entitlement is added; this is a Developer ID distribution path with hardened runtime, and the app needs user-selected filesystem/project access outside the Mac App Store sandbox model.
- **Reviewer verdict**: VERIFIED via merge commit `19e0ad0`. Clean two-track design: existing `tauri-bundles.yml` renamed to "macOS unsigned smoke app and DMG" (artifact name signals not-for-distribution); new `macos-release.yml` is the signed/notarized path. Release workflow gated by `v*` tag or `workflow_dispatch` only (no auto-push trigger), protected by `release-macos` GitHub Actions environment, with a **fail-before-build secrets check** that emits `::error title=...::` annotations if any of 7 required secrets is missing. Implementation: temporary keychain, `security import` + `set-key-partition-list` for codesign access, App Store Connect P8 written to `APPLE_API_KEY_PATH`, build via existing locked `cargo tauri build --bundles app,dmg -- --locked` (inherits R-12 CI=true + R-27 --locked), `xcrun notarytool submit --wait` then `xcrun stapler staple`, then **triple verification before upload**: `codesign --verify --deep --strict`, `spctl --assess --type execute`, `xcrun stapler validate` on both `.app` and `.dmg`, plus `spctl --assess --type open --context context:primary-signature` on `.dmg`. `tauri.conf.json` adds `"hardenedRuntime": true`. RELEASE.md updated. PKGBUILD pre-pinned `343bfe7…b7fe`. **Closes the final open R-* finding** — all 31 items resolved across the cycle.
