...
- **Branch**: `fix/m-3-pandoc-process-limits` (REOPENED — see `.review/results/M-3.reopened.md`)
...
- **Reviewer comments (REOPEN)**: Helper itself is excellent (std-only, both pipes drained concurrently, atomic counter, kill+reap on timeout/cap, 5 new tests, no zombies, all 6 pandoc callsites routed). **But the branch introduces 4 new clippy warnings (`clippy::io_other_error` at `process.rs:85, 89, 190, 212`), and `REVIEW.md`'s validation suite specifies `-D warnings`.** Under that flag the warnings become errors and future PRs stacked on M-3 will fail clippy. Fix: replace `io::Error::new(io::ErrorKind::Other, msg)` with `io::Error::other(msg)` at all 4 sites — or run `cargo clippy --fix --lib -p chickenscratch-core` for the auto-fix. Process note: please run the full validation suite from REVIEW.md before sentinelling (this is the third reopen in a row that the documented suite would have caught). Minor non-blocking nit for next pass: `OutputLimitExceeded { stream, limit }` doesn't include the observed byte count; would aid debugging.
